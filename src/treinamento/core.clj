  ...
  (doto (Properties.)
    (.putAll
     {StreamsConfig/APPLICATION_ID_CONFIG             "trt-topology"
      StreamsConfig/BOOTSTRAP_SERVERS_CONFIG          "host.docker.internal:9092"
    ...
    (case (.topic context)
      ...
      (when (= (:status msg) "pendente")
        (if (<= (:velocidade msg) 100)
          ...
          (do
            (log/warn "VEICULO MULTADO, MSG ENCAMINHADA PARA O TÓPICO: multa")
      ...
      (when (= (:status msg) "pendente")
        (cond
          (<= (:velocidade msg) 110)
          (let [valor-multa (System/getenv "VALOR_MULTA_1")]
            (log/info "MULTA " (assoc msg :status "executado" :multa valor-multa :pontos 5))
            (.forward context k (assoc msg :status "executado" :multa valor-multa :pontos 5) (To/child "cmd-radar"))
            (.forward context k (assoc msg :status "executado" :multa valor-multa :pontos 5) (To/child "cmd-relatorio")))
          (<= (:velocidade msg) 130)
          (let [valor-multa (System/getenv "VALOR_MULTA_2")]
            (log/info "MULTA " (assoc msg :status "executado" :multa valor-multa :pontos 7))
            (.forward context k (assoc msg :status "executado" :multa valor-multa :pontos 7) (To/child "cmd-radar"))
            (.forward context k (assoc msg :status "executado" :multa valor-multa :pontos 7) (To/child "cmd-relatorio")))
          (> (:velocidade msg) 130)
          (let [valor-multa (System/getenv "VALOR_MULTA_3")]
            (log/info "MULTA " (assoc msg :status "executado" :multa valor-multa :pontos 12))
            (.forward context k (assoc msg :status "executado" :multa valor-multa :pontos 12) (To/child "cmd-radar"))
            (.forward context k (assoc msg :status "executado" :multa valor-multa :pontos 12) (To/child "cmd-relatorio")))))
